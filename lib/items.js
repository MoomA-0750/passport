...
// 保存する前に、使える値かを確かめる。使おうとした瞬間に初めて壊れていると分かるのを防ぐ。
function validateSecret(field, value) {
  if (field === 'totp') {
    const totp = require('./totp');
    const problem = totp.validate(value);
    if (problem) throw new Error(`ワンタイムパスワードの設定を使えません: ${problem}`);
  }
}
...
// data/ を書き換えられると、正規アイテムの urls を攻撃者のホストに変えるだけで、
// 本物のパスワードがそのホストへ入力される（docs/crypto.md の「例外」）。
//
// そこで urls に HMAC を付ける。鍵は Vault 鍵から HKDF でアイテムごとに分けるので、
// data/ だけを持っている人には作れない。vaultId と itemId も一緒に縛り、
// 別のアイテムの HMAC を貼り替える手も落とす。

function urlsMacKey(vaultKey, itemId) {
  return envelope.hkdf(vaultKey, { salt: itemId, info: 'passport/item-urls-mac/v1' });
}

function computeUrlsMac(vaultKey, { vaultId, itemId, urls }) {
  const key = urlsMacKey(vaultKey, itemId);
  try {
    return crypto.createHmac('sha256', key)
      .update(JSON.stringify({ vaultId, itemId, urls: urls || [] }))
      .digest('base64');
  } finally {
    key.fill(0);
  }
...
function urlsMacValid(vaultKey, item) {
  if (typeof item.urlsMac !== 'string' || !item.urlsMac) return false;
  const expected = Buffer.from(computeUrlsMac(vaultKey, {
    vaultId: item.vaultId, itemId: item.id, urls: item.urls
  }), 'base64');
  const given = Buffer.from(item.urlsMac, 'base64');
...
function toPublic(item) {
  ...
  return {
    ...
    rotationDays: item.rotationDays || null,
...
// 棚卸しのリマインド用。どちらも平文のメタデータ（復号せずに期限を数えるため）。
//   rotationDays … パスワードを何日ごとに変えるか。null なら決めていない（点検では 365 日で「古い」とみなす）
//   expiresAt    … 証明書・ライセンスなどの有効期限（YYYY-MM-DD）。null なら無い
const ROTATION_CHOICES = [30, 60, 90, 180, 365, 730];

function normalizeRotationDays(value) {
  if (value === null || value === undefined || value === '' || value === 0 || value === '0') return null;
  const days = Number(value);
  if (!ROTATION_CHOICES.includes(days)) {
    throw new Error(`ローテーション周期は ${ROTATION_CHOICES.join(' / ')} 日のどれかです`);
  }
  return days;
}
...
function normalizeExpiresAt(value) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(text) || Number.isNaN(new Date(`${text}T00:00:00Z`).getTime())
    ...
    throw new Error('有効期限は YYYY-MM-DD の形で入れてください');
  }
  return text;
}
...
function create(vaultId, input, { actor }) {
  ...
  try {
    ...
    const item = {
      ...
      rotationDays: normalizeRotationDays(input.rotationDays),
      ...
      secretHistory: {},
      fieldLabels,
      urlsMac: null, // 下で付ける（id と urls が決まってから）
      createdAt: now,
      createdBy: actor,
      updatedAt: now,
      updatedBy: actor,
    ...
    item.urlsMac = computeUrlsMac(vaultKey, { vaultId, itemId: id, urls: item.urls });
...
function update(vaultId, itemId, input, { actor }) {
  ...
  try {
    const updated = store.updateJson(itemPath(vaultId, itemId), input.version, (current) => {
      ...
      if (input.rotationDays !== undefined) next.rotationDays = normalizeRotationDays(input.rotationDays);
      if (input.expiresAt !== undefined) next.expiresAt = normalizeExpiresAt(input.expiresAt);
      ...
      if (next.type === 'sshkey') {
        ...
        // 画面は編集のたびに今の公開鍵をそのまま送り返してくる。
        // 以前はそれを「公開鍵を差し替えた」と受け取り、公開鍵だけの枝でメタデータを作り直していたので、
        // タイトルを変えただけで「秘密鍵を持っている」「パスフレーズ付き」の情報が消えていた。
        // 本体（種別 + base64）が今と同じなら、差し替えではない。
        const sameKeyBody = (a, b) => String(a || '').trim().split(/\s+/).slice(0, 2).join(' ')
          === String(b || '').trim().split(/\s+/).slice(0, 2).join(' ');
        const newPublic = input.publicKey && !(current.sshKey && sameKeyBody(input.publicKey, current.sshKey.publicKey))
        ...
        if (newPrivate || newPublic) {
          ...
          if (!newPrivate && current.secrets && current.secrets.privateKey) {
            // 公開鍵だけ差し替えても、保管している秘密鍵はそのまま残っている
            next.sshKey.hasPrivateKey = true;
          }
      ...
      // 種別を sshkey 以外へ変えても、SSH のメタデータは捨てない。
      // 以前は消していたので、種別を戻そうとすると「秘密鍵か公開鍵を入れてください」と
      // 拒否され、保管済みの秘密鍵を手で取り出して貼り直すまで戻せなかった。
      // 表示側は type を見て出し分けるので、残しておいて困ることはない。

      next.updatedBy = actor;
      // urls を変えていなくても付け直す（正規の更新は、今の urls を正しいものとして扱ってよい）
      next.urlsMac = computeUrlsMac(vaultKey, { vaultId, itemId, urls: next.urls });
      return next;
    });
...
// via を渡すと、自動化トークン経由の読み出しとして記録する（actor は発行者、トークン名と ID を付ける）。
// via.client === 'cli' は人の CLI からの読み出し（記録の種別は閲覧のまま、備考に CLI と付ける）。
function revealSecret(vaultId, itemId, field, { actor, purpose = 'view', ip = null, via = null }) {
  ...
  const event = via && via.tokenId ? 'item.automation_read'
    : ({ copy: 'item.copy_secret', fill: 'item.fill_secret' }[purpose] || 'item.view_secret');
  const logged = audit.record(event, {
    actor, vaultId, itemId, field, ip,
    actorName: via && via.tokenId ? `トークン「${via.tokenName}」` : null,
    target: via && via.tokenId ? via.tokenId : null,
    note: via && via.client === 'cli' ? `${item.title}（CLI）`
      : via && via.client === 'cli-agent' ? `${item.title}（SSH エージェント）`
        : (via && via.tokenId ? `${item.title}（トークン「${via.tokenName}」${via.agent ? '・SSH エージェント' : ''}）` : item.title)
  });
  if (!logged) throw new Error('監査ログを書けないため、取り出しを止めました。管理者に連絡してください');
  return value;
}

// 点検（lib/checkup.js）のために、Vault のパスワードを1件ずつ復号して渡す。
// 値は visit の中だけで使い、返さない・保存しない。監査ログは点検側が Vault ごとにまとめて残す。
// 巻き戻し・改ざんで開けないものは value=null と理由を渡す（点検の結果に「読めない」として出す）。
//
// 平文を出す口なので、権限の確認はここで行う（呼び出し側に任せない）。
// 点検は「直せる人」だけが使うので editor 以上を求める。
// Vault の鍵を開けられないときは VaultKeyError を投げる（アイテムごとの失敗と区別するため）。
class VaultKeyError extends Error {}

function examinePasswords(vaultId, { actor }, visit) {
  if (!actor) throw new Error('examinePasswords には actor が必要です');
  const vault = vaults.requireAccess(vaultId, actor, 'editor');
  let vaultKey;
  try {
    vaultKey = vaults.unwrapKey(vault);
  } catch (err) {
    throw new VaultKeyError(err.message);
  }
  try {
    for (const item of store.listJson(`vaults/${vault.id}/items`)) {
      const record = (item.secrets || {}).password;
      if (!record) {
        visit(item, undefined, null);
        continue;
      }
      const highest = Number((item.secretCounters || {}).password || 0);
      if (highest && Number(record.v) < highest) {
        visit(item, null, 'rollback');
        continue;
      }
      let value;
      try {
        value = openSecret(vaultKey, { vaultId: vault.id, itemId: item.id, field: 'password', record });
      } catch {
        visit(item, null, 'tampered');
        continue;
      }
      visit(item, value, null);
    }
  } finally {
    vaultKey.fill(0);
  }
...
// 登録された URL のスキーム。https で登録したものは、http のページには出さない。
// 同じ LAN で DNS や ARP を握られると、平文 HTTP のページが正規のホスト名を名乗れるため。
// スキームを書かずに登録したもの（"git.example.local" だけ）は、どちらでも出す。
...
function matchHost(userId, host, { scheme = null } = {}) {
  const current = hostOf(host);
  if (!current) return [];
  const currentScheme = scheme ? String(scheme).replace(/:$/, '').toLowerCase() : null;

  const results = [];
  for (const summary of vaults.listForUser(userId)) {
    const vault = vaults.get(summary.id);
    let vaultKey;
    try {
      vaultKey = vaults.unwrapKey(vault);
    } catch {
      continue; // 鍵を開けられない Vault は候補にしない
    }
    try {
      for (const item of store.listJson(`vaults/${vault.id}/items`)) {
        ...
        // 宛先が書き換えられていないか。合わないものは、ホストが一致しても候補にしない。
        if (!urlsMacValid(vaultKey, item)) {
          audit.record('access.denied', {
            actor: userId, vaultId: vault.id, itemId: item.id, result: 'tampered',
            note: `${item.title}: URL の完全性を確かめられないので、オートフィルの候補から外した`
          });
          continue;
        }
        ...
        for (const url of item.urls || []) {
          const registeredScheme = schemeOf(url);
          if (currentScheme === 'http' && registeredScheme === 'https') continue;
