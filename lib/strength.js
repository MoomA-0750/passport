...
function weaknesses(password) {
  ...
  // 末尾の数字・記号を落とす。正規表現の $ 付きの繰り返しは、長い値で処理時間が長さの2乗になり
  // 単一プロセスのサーバー全体を止めるので、後ろから1文字ずつ見る。よくある語との比較は先頭 64 文字で足りる
  const lower = text.slice(0, 64).toLowerCase();
  let end = lower.length;
  while (end > 0 && TRAILING.has(lower[end - 1])) end -= 1;
  const core = lower.slice(0, end);
