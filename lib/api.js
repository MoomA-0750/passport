...
route('POST', '/api/tokens', async (ctx) => {
  requireLogin(ctx);
  const { name, vaultIds, expiresInDays } = ctx.body;
...
function requireReader(ctx) {
  ...
  if (ctx.user && ctx.viaBearer) {
    const client = ctx.session && ctx.session.kind === 'extension' ? 'extension' : 'cli';
    if (client === 'cli' && !config.cli.enabled) throw new ApiError(403, 'CLI はサーバーの設定で止めてあります');
...
function readerVia(reader) {
  return reader.kind === 'token'
    ? { tokenId: reader.token.id, tokenName: reader.token.name }
...
route('POST', '/api/automation/vaults/:vaultId/items/:itemId/read', async (ctx) => {
  ...
  const value = items.revealSecret(ctx.params.vaultId, ctx.params.itemId, field, {
    actor: reader.userId, purpose: 'view', ip: ctx.ip, via: readerVia(reader)
  });
  return { field, value };
});
...
route('GET', '/api/me/activity', async (ctx) => {
  ...
  return { entries: activity.forUser(ctx.user.id, { limit }) };
});
...
route('GET', '/api/vaults/:vaultId/members', async (ctx) => {
  ...
  const candidates = isOwner
    ...
    : [];
  // 共有先に足せるグループ。足す前の段階では名前と人数だけ（中の人は出さない）
