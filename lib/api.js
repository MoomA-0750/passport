...
function requireReader(ctx) {
  ...
  if (ctx.user && ctx.viaBearer) {
    const client = ctx.session && ctx.session.kind === 'extension' ? 'extension'
      : (String(ctx.req.headers['x-passport-client'] || '').toLowerCase() === 'cli-agent' ? 'cli-agent' : 'cli');
    if (client !== 'extension' && !config.cli.enabled) throw new ApiError(403, 'CLI はサーバーの設定で止めてあります');
