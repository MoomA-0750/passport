...
const config = {
  ...
  cli: {
    // 人の CLI のログイン（X-Passport-Client: cli で Bearer を受け取る）を許すか。
    // 拡張と違って呼び出し元を絞る手段が無い（パスワードを知っていれば、どの端末からでも最長 24 時間の Bearer を作れる）ので、
    // 使わないなら切る。切ってもブラウザの画面には影響しない。
    enabled: toBool(process.env.PASSPORT_CLI !== undefined ? process.env.PASSPORT_CLI : cl.enabled, true),
    // 自動化トークンを発行・使用できるか。切ると、発行済みのトークンも使えなくなる。
    tokens: toBool(process.env.PASSPORT_AUTOMATION_TOKENS !== undefined ? process.env.PASSPORT_AUTOMATION_TOKENS : cl.tokens, true)
  }
};
