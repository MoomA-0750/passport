...
//   ・その中だけで比べ、結果もその中だけ（ほかの Vault との一致は出さない）
...
function run(userId, { ip = null, now = Date.now() } = {}) {
  ...
  const byFingerprint = new Map(); // HMAC -> [ref]
  const weak = [];
  const old = [];
  const unreadable = [];
  const due = [];
  ...
  const reused = [...byFingerprint.values()].filter((group) => group.length > 1);
  byFingerprint.clear();
  ...
  for (const group of reused) {
    for (const ref of group) {
      ...
      reusedIdsByVault.get(ref.vaultId).push(`${ref.title}(${ref.itemId.slice(0, 8)})`);
    }
  }
  for (const [vaultId, counts] of perVault) {
    if (counts.examined === 0 && counts.unreadable === 0) continue;
    const reusedTitles = reusedIdsByVault.get(vaultId) || [];
    const logged = audit.record('vault.checkup', {
      ...
      note: `調べた ${counts.examined} / 弱い ${counts.weak} / 古い ${counts.old} / 読めない ${counts.unreadable}`
        + (reusedTitles.length ? ` / 使い回し: ${reusedTitles.join('、').slice(0, 800)}` : '')
    });
