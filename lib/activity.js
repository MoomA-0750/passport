...
const VAULT_EVENT_PREFIXES = ['item.', 'vault.', 'token.'];
// 1回の表示で読む監査ログの上限。これより古い分は「さらに古い記録は管理者の監査ログで」と出す
const MAX_SCAN_BYTES = 32 * 1024 * 1024;

function forUser(userId, options = {}) {
  return forUserWithInfo(userId, options).entries;
}

function forUserWithInfo(userId, { limit = 200, months = 3 } = {}) {
  const me = users.get(userId);
  if (!me) return { entries: [], truncated: false };
  ...
  const { entries, truncated } = audit.scanWithInfo({ limit, months, maxBytes: MAX_SCAN_BYTES, filter: (entry) => !!relationOf(entry) });
  ...
  // 備考には記録した時点のアイテム名や Vault 名が入っている。今入っていない Vault の記録では出さない
  // （外されたあとも、名前が備考経由で見え続けないように）
  function noteOf(entry) {
    if (entry.vaultId && !accessible.has(entry.vaultId)) return null;
    if (entry.event === 'group.member_add' && entry.note) return entry.note.split(' → ')[0];
    return entry.note;
  }
}

module.exports = { forUser, forUserWithInfo };
