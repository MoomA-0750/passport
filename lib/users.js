...
// 照合にかける時間をそろえるための、形だけのハッシュ。
// 「居ないユーザー」「無効化されたユーザー」「止められている組み合わせ」の応答が
// 正規の照合より速いと、応答時間からユーザー名の存在や状態を当てられる。
const DUMMY_HASH = {
...
function burnTime(password) {
  envelope.verifyPassword(String(password || ''), keyring.pepper(), DUMMY_HASH);
}

// ログイン。戻り値は { ok, user, reason }。
//
// reason を画面にそのまま出さないこと。どの失敗でも、利用者への応答は同じにする
// （ユーザー名の存在・無効化・ロックの別を、応答の中身や時間から当てられないように）。
//   invalid  … ユーザー名かパスワードが違う／無効化されている
//   throttled … この組み合わせ（または接続元）からの失敗が多すぎる
function authenticate(username, password, { ip = null } = {}) {
  const throttle = require('./throttle');

  // 失敗の数え方は「ユーザー名 × 接続元」。以前のユーザー単位のロックは、
  // 誰でも特定の人を締め出せる状態だった（lib/throttle.js の説明を読む）。
  const gate = throttle.check(username, ip);
  if (!gate.allowed) {
    burnTime(password);
    audit.recordSampled(`login|${ip}|throttled|${gate.reason === 'ip' ? '-' : attemptMark(username)}`, 'login.fail', {
      ip, result: 'throttled',
      note: gate.reason === 'ip' ? '接続元からの失敗が多すぎる' : `試行の印 ${attemptMark(username)}`
    });
  ...
  if (!user) {
    ...
    audit.recordSampled(`login|${ip}|no_such_user|${attemptMark(username)}`, 'login.fail', { ip, result: 'no_such_user', note: `試行の印 ${attemptMark(username)}` });
  ...
  if (user.status !== 'active') {
    burnTime(password);
    throttle.recordFailure(username, ip);
    audit.recordSampled(`login|${ip}|disabled|${user.id}`, 'login.fail', { actor: user.id, actorName: user.username, ip, result: 'disabled' });
    return { ok: false, reason: 'invalid' };
  }
  ...
  if (!ok) {
    const { lockedNow } = throttle.recordFailure(username, ip);
    audit.record(lockedNow ? 'login.locked' : 'login.fail', {
      actor: user.id, actorName: user.username, ip, result: 'bad_password',
      note: lockedNow ? `この接続元から${config.auth.lockoutMinutes}分止めた` : null
    });
    return { ok: false, reason: 'invalid' };
  }
  ...
  // 成功。必要ならハッシュのパラメータを上げ直す。
  // 以前のユーザー単位のロック（failedAttempts / lockedUntil）は使わなくなったので消しておく。
...
// ログイン中の本人に、もう一度パスワードを確かめる（自動化トークンの発行など、長く効くものを作る前）。
// 盗まれたセッション（拡張の Bearer や CLI の session.json）だけでは、長寿命の入口を作れないようにする。
// 失敗はログインと同じく「ユーザー名 × 接続元」で数える。
function confirmPassword(id, password, { ip = null } = {}) {
  const throttle = require('./throttle');
  const user = get(id);
  if (!user || user.status !== 'active') return false;
  if (!throttle.check(user.username, ip).allowed) {
    burnTime(password);
    return false;
  }
  const ok = envelope.verifyPassword(String(password || ''), keyring.pepper(), user.passwordHash);
  if (ok) throttle.recordSuccess(user.username, ip);
  else throttle.recordFailure(user.username, ip);
  return ok;
}
...
function resetPassword(id, newPassword, { actor = null } = {}) {
  ...
  require('./throttle').clearUser(user.username);
