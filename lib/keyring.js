...
//   1. キーファイル（既定）: /etc/keybox/master.key を 0400 で置く。systemd で自動起動できる
//   2. パスフレーズ: 環境変数 KEYBOX_MASTER_PASSPHRASE。起動ごとに人手でアンロックする運用
//
...
const log = require('./logger')('keybox:crypto');

// パスフレーズ運用のときの固定ソルト。
// パスフレーズはユーザーごとではなくサーバーに1つなので、ソルトを設定に持たせる意味がなく、
// 用途ラベルとして固定する。強度はパスフレーズの長さで確保する。
const PASSPHRASE_SALT = Buffer.from('keybox/master-passphrase/v1', 'utf8');
...
function deriveFromPassphrase(passphrase) {
  if (String(passphrase).length < 16) {
    throw new Error('KEYBOX_MASTER_PASSPHRASE は16文字以上にしてください');
  }
...
function unlock({ allowGenerate = false } = {}) {
  ...
  if (config.crypto.masterPassphrase) {
    ...
    if (!fs.existsSync(file)) {
      ...
      log.warn('このファイルのバックアップを、KeyBox のデータとは別の場所に取ってください');
      source = 'file(new)';
    } else {
  ...
  state = {
    kek: envelope.hkdf(master, { info: 'keybox/kek/v1' }),
    pepper: envelope.hkdf(master, { info: 'keybox/pepper/v1' }),
    source
  };
...
function selfTest() {
  ...
  const sealed = envelope.seal(k, 'keybox-selftest', aad);
  const opened = envelope.open(k, sealed, aad);
  if (opened !== 'keybox-selftest') {
