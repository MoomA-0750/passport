...
const log = require('./logger')('keybox:store');
...
function vaultKeyAad(vaultId, holder) {
  return Buffer.from(`keybox/vault-key|${vaultId}|${holder}`, 'utf8');
}
...
function adminTakeOwnership(vaultId, { actor, reason }) {
  ...
  const updated = store.updateJson(vaultPath(vaultId), null, (current) => {
    ...
    if (existing) {
      return { ...current, members: current.members.map((m) => (m.userId === actor ? { ...m, role: 'owner' } : m)) };
    }
    return { ...current, members: [...current.members, { userId: actor, role: 'owner', addedAt: now, addedBy: actor, viaBreakGlass: true }] };
  });
