...
function addMember(vaultId, { userId, role = 'viewer', actor }) {
  ...
  return audit.recordBefore('vault.member_add', { actor, vaultId, target: userId, note: role }, () =>
    store.updateJson(vaultPath(vaultId), null, (current) => ({
      ...current,
...
function updateMember(vaultId, { userId, role, actor }) {
  ...
  if (role !== 'owner' && directOwners(vault, userId).length === 0) {
    throw new Error('最後の owner の権限は下げられません。先に別の owner を追加してください');
  }
  return audit.recordBefore('vault.member_update', { actor, vaultId, target: userId, note: role }, () =>
    store.updateJson(vaultPath(vaultId), null, (current) => {
...
function addGroup(vaultId, { groupId, role = 'viewer', actor }) {
  ...
  return audit.recordBefore('vault.group_add', { actor, vaultId, target: groupId, note: `${group.name} (${role})` }, () =>
    store.updateJson(vaultPath(vaultId), null, (current) => ({
      ...current,
...
function updateGroup(vaultId, { groupId, role, actor }) {
  ...
  return audit.recordBefore('vault.group_update', { actor, vaultId, target: groupId, note: role }, () =>
    store.updateJson(vaultPath(vaultId), null, (current) => ({
      ...current,
...
function adminTakeOwnership(vaultId, { actor, reason }) {
  ...
  const now = new Date().toISOString();
  const updated = audit.recordBefore('vault.member_add', {
